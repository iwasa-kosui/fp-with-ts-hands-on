...
const printWithAdditionalStartExaminationError = (
  fileName: string,
  source: string,
): string => {
    ...
    (context) => {
      const visit: ts.Visitor = (node) => {
        if (
          ...
          node.name.text === "StartExaminationError" &&
            ...
            ts.factory.createTypeLiteralNode([
              ts.factory.createPropertySignature(
                ...
                ts.factory.createLiteralTypeNode(
                  ts.factory.createStringLiteral("AppointmentUnavailable"),
                ),
              ),
            ]),
          ]);
  ...
  if (outputSource === undefined) {
    throw new Error("StartExaminationError transform produced no source file");
  }
...
const printWithExpectedStartExaminationExhaustivenessError = (
  fileName: string,
  source: string,
...
export const compileWithAdditionalStartExaminationError = (): ReadonlyArray<string> => {
  const config = ts.readConfigFile(configPath, ts.sys.readFile);
  const parsed = ts.parseJsonConfigFileContent(config.config, ts.sys, projectDirectory);
  ...
  host.readFile = (fileName) => {
    ...
    if (fileName === errorsPath) {
      return printWithAdditionalStartExaminationError(fileName, source);
    }
    if (fileName === routesPath) {
      const transformed = printWithExpectedStartExaminationExhaustivenessError(
        fileName,
        source,
      );
  ...
  return foundAssertNever
    ? diagnostics
    : ["toStartExaminationNoticeCode must return assertNever(error) by default"];
};
