...
describe.each(snapshotScenarios)("$name", (scenario) => {
  test("persists the common check-in and examination-start outcome", async () => {
    ...
    try {
      auditLogCountBeforeStart = observeAppointment(databasePath, clinicFixture.appointmentId).auditLogs.length;
    ...
    expect(observation.auditLogs.length).toBeGreaterThan(auditLogCountBeforeStart);
...
test("Session 00 accepts a restart after payment and persists the restarted state", async () => {
  ...
  try {
    ...
    expect(scenario.normalizeState(observeAppointment(databasePath, clinicFixture.appointmentId).state)).toBe(
      "Paid",
    );
    expect((await post(app, `${appointmentUrl}/start-examination`)).status).toBe(303);
  } finally {
