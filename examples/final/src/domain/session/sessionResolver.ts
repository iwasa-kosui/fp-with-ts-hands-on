...
export type SessionByIdResolver = Readonly<{
  resolveById: (sessionId: SessionId) => ResultAsync<Session | undefined, never>;
}>;

export type SessionByTokenHashResolver = Readonly<{
  resolveByTokenHash: (tokenHash: SessionTokenHash) => ResultAsync<Session | undefined, never>;
}>;

export type SessionByUserIdResolver = Readonly<{
  resolveByUserId: (userId: UserId) => ResultAsync<readonly Session[], never>;
}>;
