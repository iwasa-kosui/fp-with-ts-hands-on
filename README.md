...
公開教材は、現行業務とシステムを読む S0、業務イベントから集約を見つける S1、コードを改善する S2〜S6、講師ツアーの Final で構成します。S0 は10分のオリエンテーションです。S1 は15分の班ワークで、担当者ごとの記録からドメインイベントを拾い、予約患者と急患のレーンへ置いて、集約と未解決の責任を Excalidraw 上で見つけます。S1 ではコード編集や exercise command を行いません。S2〜S6 は各30分のコード演習で、各starterは業務語彙を含む assertion failure による RED から始まります。Final は5分です。
...
2. S1 で担当者ごとの記録からドメインイベントを拾い、患者別レーンで集約と未解決の責任を見つける
