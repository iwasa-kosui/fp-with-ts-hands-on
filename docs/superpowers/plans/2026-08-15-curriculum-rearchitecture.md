...
**Interfaces:**
- Consumes: P1 の実ファイル、step solution、実測差分。
...
テストは設計 §6.3 の 16 件を固定する。特に合計 150 分、exercise と optional fields の同値、`peerReview.minutes === timeBreakdown.review`、3 問、全参照ファイルと `solution.symbol` の実在を literal ではなくデータから検証する。
...
`StepSolution` は solution path と line range から実ソースを切り出し、`<details>` に goal と code を描画する。`PeerReviewPanel` は `N分・1〜2名`、3 問、S1 の約束事へのリンクを静的 HTML として描画する。新しい client island や依存を追加しない。
