...
- S1はExcalidrawを使い、担当者ごとの記録から拾ったドメインイベントを予約患者と急患のレーンへ置き、集約と未解決の責任を班で見つけます。実行可能な演習ではありません。
- S2〜S5は各回のfocusであるcurrent state、input、expected failures、output event / side effectsに残ったリスクを1回に1つずつ実装する。
...
- S1の15分で、班が3人の担当者の記録から7件のドメインイベントを拾い、ムギとソラのレーンへ置いて、患者ごとの集約にまとめられるか。
- 4つの語彙に加えて、ムギの診察保留を記録して受付へ伝える責任を「?」として残す作業まで、6分の班ワークで扱いきれるか。
- ExcalidrawのLive collaborationを班ごとに立ち上げる手順が、S1の3分のブリーフィング中に完了するか。
