...
- S1ではExcalidrawを使い、事前配置された「予約がキャンセルされた」からアクター、コマンド、業務条件を逆算します。講師はROP基本図で3か所の失敗への切替を示し、別の成功経路図で `Appointment.cancel` を内部のドメイン判断へ置き、入力側のResolverと出力側のEvent Storeを確認します。実行可能な演習ではありません。
...
- Event、Actor、Command、Hotspotの4種類だけを参加者が扱い、講師が後から `Appointment` 集約とROPを加える進行で混乱しないか。
