...
- S1はExcalidrawを使い、散文から拾ったドメインイベントから集約を班で見つける。実行可能な演習ではない。
...
- S1の15分で、班が散文からドメインイベントを拾い、コマンドとアクターを添えて、集約にまとめられるか。
- 語彙が3つから4つ（ドメインイベント・コマンド・ワークフロー・集約）に増え、6分の班ワークで扱いきれるか。
