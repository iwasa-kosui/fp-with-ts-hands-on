...
カリキュラム同期日: 2026-08-29（Asia/Tokyo）
...
- S1はExcalidrawを使い、事前配置された「診察が開始された」からアクター、コマンド、事前条件を逆算する。事前条件が決まった後に、講師が予約集約とワークフローの入出力を示す。実行可能な演習ではない。
...
- S1の15分で、班が「診察が開始された」からアクター、コマンド、事前条件を4分で逆算できるか。
- Event、Actor、Command、Hotspotの4種類だけを参加者が扱い、講師が後から予約集約を加える進行で混乱しないか。
