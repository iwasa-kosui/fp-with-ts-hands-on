...
コードを編集する演習は S2〜S5 の4本です。S1 は紙面またはページ上のワークフローカードを使い、ビジネスイベントと診察開始ワークフローを班で描く回です。コマンドは実行しません。
...
S0 は現状の業務・操作・保存先・ログを読む回、S1 はワークフローカードを使う班ワークなので演習コマンドはありません。`examples/session-06` は全解答と回帰テストを持つ到達点です。Final は講師が参照実装を案内する5分のツアーで、参加者が `examples/final` をセットアップしたり編集したりする必要はありません。
